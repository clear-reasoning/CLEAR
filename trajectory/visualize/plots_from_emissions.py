...
Generate emissions for all trajectories with an RL controller
    ...
    python simulate.py --cp_path checkpoints/gamma=0.99_gae_lambda=0.9_env_num_concat_states=1_env_horizon=1000/checkpoints/800.zip
    --av_controller rl --all_trajectories --gen_emissions
...
for path in emissions_folders:
    for csv_path in path.glob('*.csv'):
        ...
        # data[traj_name + '1'][av_controller] = data[traj_name][av_controller]
        # data[traj_name + '2'][av_controller] = data[traj_name][av_controller]
        # data[traj_name + '3'][av_controller] = data[traj_name][av_controller]
...
for traj_name, dfs in data.items():
    for av_controller, df in dfs.items():
        ...
        plot.add(top=leader_speeds.max(), bottom=leader_speeds.min(),
                 group=traj2idx[traj_name], label=av_controller, subplot='leader_speeds')
        ...
        plot.add(top=max(av_headways_max), bottom=min(av_headways_min),
                 group=traj2idx[traj_name], label=av_controller, subplot='avs_headways')
        ...
        plot.add(top=max(av_accels_max), bottom=min(av_accels_min),
                 group=traj2idx[traj_name], label=av_controller, subplot='avs_accels')
        ...
        plot.add(top=np.mean(mpg_humans + mpg_avs),
                 group=traj2idx[traj_name], label=av_controller, subplot='mpg_avs_humans')
...
plot.configure_subplot(subplot='avs_accels', title='Acceleration range of AVs (w/ failsafes)',
...
for max_speed, av_controllers in mpgs_by_speed.items():
    for av_controller, mpgs in av_controllers.items():
        print(f'{av_controller} \t {max_speed-10}m/s to {max_speed}m/s \t avg mpg: {round(np.mean(mpgs), 2)}\t',
              np.sum(steps_by_speed[max_speed][av_controller]), 'steps')
...
# idm_emissions_path = sys.argv[1]
# av_emissions_path = sys.argv[2]

# df_idm = pd.read_csv(idm_emissions_path)
# df_rl = pd.read_csv(av_emissions_path)

# df_idm_av = df_idm.loc[df_idm['id'].isin(['1_idm_av', '1_rl_av'])]
# df_rl_av = df_rl.loc[df_rl['id'].isin(['1_idm_av', '1_rl_av'])]

# plots = [('idm', df_idm_av), ('rl', df_rl_av)]

# plotter = Plotter('figs/controller_analysis')

# for key in ['speed', 'accel', 'speed_difference', 'headway']:
#     with plotter.subplot(title=key, xlabel='Time (s)', grid=True, legend=True):
#         for lbl, df in plots:
#             plotter.plot(df['time'], df[key], label=lbl)

# for N in [100, 1000, 5000]:
#     with plotter.subplot(title=f'Running MPGs over the last {N//10}s', xlabel='Time (s)', ylabel='MPG average', grid=True, legend=True):
#         for lbl, df in plots:
#             df_running_sum = df.rolling(N).sum()
#             mpgs = (df_running_sum['speed'] / 1609.34) / (df_running_sum['instant_energy_consumption'] / 3600 + 1e-6)
#             plotter.plot(df['time'], mpgs, label=lbl)

# with plotter.subplot(title=f'Cumulative MPG', xlabel='Time (s)', ylabel='Cumulative MPG', grid=True, legend=True):
#     for lbl, df in plots:
#         mpgs = (np.cumsum(df['speed']) / 1609.34) / (np.cumsum(df['instant_energy_consumption']) / 3600 + 1e-6)
#         plotter.plot(df['time'], mpgs, label=lbl)

# for lbl, df in plots:
#     print(lbl, 'mpg', np.array(df['avg_mpg'])[-1])

# plotter.save(f'data', log='\t')
