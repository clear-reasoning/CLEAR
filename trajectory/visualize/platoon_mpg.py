...
def plot_platoon_mpg(emissions_path, save_path):
    ...
    ax1.add_patch(Rectangle((len(id_dict) - av_nums[-1] - 0.5, 0), av_nums[3] -
                  av_nums[2] - 1, max_mpg * 1.1, edgecolor='g', fill=False, lw=2))
    plt.annotate('Platoon 4', (len(id_dict) - av_nums[-1], max_mpg * 1.2), fontsize=16)
    ax1.add_patch(Rectangle((len(id_dict) - av_nums[2] - 0.5, 0), av_nums[2] -
                  av_nums[1] - 1, max_mpg * 1.1, edgecolor='g', fill=False, lw=2))
    plt.annotate('Platoon 3', (len(id_dict) - av_nums[2], max_mpg * 1.2), fontsize=16)
    ax1.add_patch(Rectangle((len(id_dict) - av_nums[1] - 0.5, 0), av_nums[1] -
                  av_nums[0] - 1, max_mpg * 1.1, edgecolor='g', fill=False, lw=2))
    plt.annotate('Platoon 2', (len(id_dict) - av_nums[1], max_mpg * 1.2), fontsize=16)
    ax1.add_patch(Rectangle((len(id_dict) - av_nums[0] - 0.5, 0),
