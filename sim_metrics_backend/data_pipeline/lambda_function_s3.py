...
from run.data_pipeline import AthenaQuery, delete_obsolete_data, update_baseline, get_ready_queries, get_completed_queries, put_completed_queries
from run.query import tables, network_filters, summary_tables, triggers, max_decel, leader_max_decel
...
def lambda_handler(event, context):
    ...
    for bucket, key, table, query_name, query_date, partition, source_id in records:
        ...
        # Get the network-depend parameters
        ...
        if 'transfer_test_name' in response["Metadata"]:
            if query_name == "LEADERBOARD_CHART":
                ...
                if transfer_simulation_name not in completed_transfer_queries:
                    completed_transfer_queries.add(transfer_simulation_name)
