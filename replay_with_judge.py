...
CHUNK_COL_CSV = "step"
PLOT_RESULTS = True
MAX_RETRIES = 5 # number of back and forth actions
...
async def evaluate_window_async(window_data, start_idx, llm_agent, judge_agent=None, semaphore=None):
    async with semaphore:
        ...
        for attempt in range(MAX_RETRIES):
            response = await llm_agent.get_response_async(SYSTEM_PROMPT, user_prompt, temperature=0.7)

            verdict = await verify_response(response, judge_agent)
            if verdict["verdict"] != "yes":
                logging.warning(f"[Attempt {attempt+1}] Judge rejected best response: {verdict}")
                continue

            speed_vals, speed_ok = extract_values(response, "future_speeds")
            headway_vals, headway_ok = extract_values(response, "future_headway")
            leader_vals, leader_ok = extract_values(response, "future_leader_speed")
            reward_vals, reward_ok = extract_values(response, "future_reward")
...
async def main():
    ...
    tasks = [
        evaluate_window_async(window, window.iloc[0][chunk_col], llm_agent, judge_agent, semaphore)
